...
BarWidget {
  ...
  // Qt has no es_ES locale generated on this system (see `locale -a`), so
  // Qt.formatDateTime always spells day/month names in English regardless of
  // format string. Translate the handful of tokens the format ring actually
  // uses rather than generating a system locale for one clock label.
  readonly property var esNames: ({
    ...
    "Jan": "ene", "Feb": "feb", "Apr": "abr", "Jun": "jun", "Jul": "jul",
    "Aug": "ago", "Sep": "sep", "Oct": "oct", "Nov": "nov", "Dec": "dic"
  })
  readonly property var esNamesPattern: new RegExp("\\b(" + Object.keys(esNames).join("|") + ")\\b", "g")

  function translateEs(text) {
    return text.replace(esNamesPattern, function(match) { return esNames[match] })
  }

  function formatted(date) {
    var raw = Qt.formatDateTime(date, activeFormat.replace(/ww/g, Model.isoWeekLiteral(date.getFullYear(), date.getMonth(), date.getDate())))
    return translateEs(raw)
  }
